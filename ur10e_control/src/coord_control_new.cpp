...
int main(int argc, char **argv) {
  ...
  static const std::string PLANNING_GROUP = "ur10e_arm";
  ...
  moveit_visual_tools::MoveItVisualTools visual_tools(
      move_group_node, "ur10e_base_link", "move_group_tutorial",
  ...
  if (!within_bounds) {
    RCLCPP_WARN(LOGGER, "Target joint position(s) were outside of limits, but "
                        "we will plan and clamp to the limits ");
  }
  ...
  ocm.link_name = "ur10e_tool0";
  ocm.header.frame_id = "ur10e_base_link";
  ...
  RCLCPP_INFO(LOGGER, "Visualizing plan 4 (Cartesian path) (%.2f%% achieved)",
              fraction * 100.0);
  ...
  // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  //
  // You can attach an object to the robot, so that it moves with the robot
  // geometry. This simulates picking up the object for the purpose of
  // manipulating it. The motion planning should avoid collisions between
  // objects as well.
  ...
  // that the object is allowed to be in collision with the finger links of the
  // gripper. You could also use applyAttachedCollisionObject to attach an
  ...
  visual_tools.deleteAllMarkers();
  visual_tools.publishText(text_pose, "Object_detached_from_robot", rvt::WHITE,
                           rvt::XLARGE);
  visual_tools.trigger();
